...
def test_create_tally():
    #without the tally created, trying to alter it forces a name error as it doesn't yet exist...
    with pytest.raises(NameError):
        alter_tally('right')
    create_tally()
    assert alter_tally('right') == {'up': 0, 'down': 0, 'left': 0, 'right': 1}

def test_alter_tally():
    create_tally()
    assert alter_tally('right') == {'up': 0, 'down': 0, 'left': 0, 'right': 1}
    assert alter_tally('right') == {'up': 0, 'down': 0, 'left': 0, 'right': 2}
...
def test_musk_move():
    set_board(board1)
    assert musk_move() == ((2, 2), 'left')

def test_rich_move():
    set_board(board10)
    tally = create_tally()
    alter_tally('right')
    assert rich_move() == ((0, 3), 'right')
    alter_tally('left')
    alter_tally('left')
    assert rich_move() == ((4, 1), 'left')

def test_m_positions():
    set_board(board1)
    assert m_positions() == [(0, 3), (1, 3), (2, 2)]

def test_get_move_values():
    set_board(board1)
    assert get_move_values() == {((1, 3), 'down'): 6, ((1, 3), 'left'): 6, ((2, 2), 'up'): 4, ((2, 2), 'left'): 8, ((2, 2), 'right'): 4}

def test_get_enemy_move_values():
    set_board(board1)

    assert get_enemy_move_values() ==  {6.0: [((1, 2), 'up'), ((1, 2), 'left'), ((2, 3), 'down'), ((2, 3), 'right'), ((3, 1), 'right')], 5.0: [((2, 1), 'up'), ((2, 1), 'left')], 5.6: [((3, 1), 'down'), ((3, 1), 'left'), ((4, 3), 'up'), ((4, 3), 'left'), ((4, 3), 'right')]}

def test_backup():
    set_board(board1)
    board1[0][0] = 'this is a test'
    assert board1 == [ ['this is a test', _, _, M, _],
                [_, _, R, M, _],
                [_, R, M, R, _],
                [_, R, _, _, _],
                [_, _, _, R, _] ]
    assert backup() == [ [_, _, _, M, _],
                [_, _, R, M, _],
                [_, R, M, R, _],
                [_, R, _, _, _],
                [_, _, _, R, _] ]
